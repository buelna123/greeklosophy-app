...
const getFrasesPorRuta = (pathname: string): string[] => {
  ...
  if (pathname.startsWith("/articles")) return frasesArticulos;
  if (pathname.startsWith("/dashboard")) return frasesDashboard;
  if (pathname.startsWith("/admin")) return frasesAdmin;
  if (pathname === "/") return frasesHome;
...
export const FraseMarquesina = () => {
  ...
  return (
    <div className="marquesina-container" ref={containerRef}>
      <div className="marquesina-inner">
        <div className="marquesina-track" ref={trackRef}>
          {[...frases, ...frases, ...frases].map((frase, index) => (
            <span key={index} className="marquesina-frase">
              {frase}
            </span>
          ))}
        </div>
